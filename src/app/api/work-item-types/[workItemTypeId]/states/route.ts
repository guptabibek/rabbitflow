...
const transitionSchema = z.object({
  ...
  approverRoles: z.array(z.enum(['Admin', 'PM', 'DevOps', 'Dev', 'QA', 'Viewer'])).optional(),
...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ workItemTypeId: string }> }
) {
  try {
    const { workItemTypeId: id } = await params
      ...
      if (data.transitions.length > 0) {
        await tx.stateTransition.createMany({
          data: data.transitions.map((transition, index) => ({
            ...
            minApprovals: transition.minApprovals ?? 1,
          })),
          skipDuplicates: true,
        })
      }
